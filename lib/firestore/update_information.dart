...
Future<void> updateInformation({
  ...
  required PasswordStrengthChecker passwordChecker,
  ...
  if (fieldsToSubmit.contains(option)) {
    if (await isExists(firestore, collectionName,
        controllers[name]?.toString() ?? '', option, userId)) {
      if (!context.mounted) return;
  ...
  if (fieldsToSubmit.contains(option1)) {
    if (await isExists1(firestore, collectionName,
        controllers[name1]?.toString() ?? '', option1, userId)) {
      if (!context.mounted) return;
  ...
  try {
    ...
    if (!context.mounted) return;
    showCustomDialog(
        context, 'Successful', 'Information Updated Successfully.');
    if (!context.mounted) return;
...
Future<bool> isExists(FirebaseFirestore firestore, String collectionName,
    String name, String option, dynamic userId) async {
  try {
...
Future<bool> isExists1(FirebaseFirestore firestore, String collectionName,
    String name1, String option1, dynamic userId) async {
  try {
    var querySnapshot = await firestore
        .collection(collectionName)
